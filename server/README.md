...
เชิงภาพละเอียดกว่า tier "Flash" ที่ออกแบบมาเพื่อความเร็ว/ต้นทุนต่ำเป็นหลัก `gemini-2.5-pro` เป็น GA stable
model ID ของ Google เอง (release 2025-06-17, สนับสนุนถึงอย่างน้อย 2026-10-16) ไม่ใช่ alias แบบ "-latest" ที่
สลับโมเดลใต้ชื่อเดิมได้ และรุ่น 2.5 นี้ไม่มี dated snapshot (`-001`/`-002`) แยกให้ตรึงเพิ่มเหมือนโมเดลรุ่นก่อน —
บันทึกชื่อ `gemini-2.5-pro` พร้อมวันที่ deploy จริงไว้ในบทระเบียบวิธีของงานวิจัยก็เพียงพอสำหรับการทำซ้ำผล
(ปรับ `--timeout` เป็น 120s เพราะ Pro ตอบช้ากว่า Flash)
